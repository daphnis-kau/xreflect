...
Base::CApp* Base::CApp::s_pInstance = nullptr;

Base::CApp& Base::CApp::GetInstance()
{
	return *s_pInstance;
}
...
CApplication::CApplication() : m_pHandler( nullptr ) {}
...
TestMap& CApplication::TestTemplateMap( int nKey, const char* szValue )
{
	m_mapTest[nKey] = szValue;
	return m_mapTest;
}
...
SApplicationConfig CApplication::TestCallObjectValue( SApplicationConfig Config )
{
	...
	auto pID = XReflect::TClass<SApplicationConfig>::GetClass()->FindField( "nID" );
	auto pName = XReflect::TClass<SApplicationConfig>::GetClass()->FindField( "szName" );
	pID->Set<SApplicationConfig, uint32>( &Config, 54321 );
	pName->Set<SApplicationConfig, const char*>( &Config, "access" );
	Test( !strcmp( pName->Get<SApplicationConfig, const char*>( &Config ), "access" ) &&
		pID->Get<SApplicationConfig, uint32&>( &Config ) == 54321,
		"Test get set object value by reflection" );
	Config.nID = 12345;
	strcpy( Config.szName, "sampler" );
