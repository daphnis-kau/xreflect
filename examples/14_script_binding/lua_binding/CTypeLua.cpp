...
namespace ScriptBinding
{
	...
	//=====================================================================
	/// Encapsulation of Lua operations on C++ data
	//=====================================================================
	...
	void CLuaObject::GetFromVM( UPakDataType Type, lua_State* pL, 
		char* pDataBuf, int32 nStkId, int32 nWeakTable )
	{
		...
		auto pDesClass = Type.GetClass();
		if( pDesClass == nullptr )
			return;
		...
		if( nType == LUA_TTABLE )
		{
			...
			tbyte* pObj = (tbyte*)lua_touserdata( pL, -1 );
			lua_pushlightuserdata( pL, CScriptLua::ms_pCppClass );
			lua_rawget( pL, nStkId );
			auto pSrcClass = (const CClass*)lua_touserdata( pL, -1 );
			auto nOffset = pSrcClass->CalculateBaseOffset( pDesClass );
			*(void**)( pDataBuf ) = nOffset >= 0 ? pObj + nOffset : nullptr;
			lua_pop( pL, 2 );
		}
		else
		{
	...
	void CLuaObject::PushToVM( UPakDataType Type, 
		lua_State* pL, char* pDataBuf, int32 nWeakTable )
	{
		...
		if( !lua_isnil( pL, -1 ) )
		{
			...
			if( nType == LUA_TUSERDATA || nType == LUA_TLIGHTUSERDATA )
			{
				lua_pushlightuserdata( pL, CScriptLua::ms_pCppClass );
				lua_rawget( pL, -3 );			//[Obj, CppObj, Class]
				auto pOrgClass = (const CClass*)lua_touserdata( pL, -1 );
				if( pOrgClass->IsA( pClass ) )
				{
					lua_pop( pL, 2 );
					return;
				}
		...
		// Table is at the top of the Lua stack
		lua_newtable( pL );
		...
		if( lua_isnil( pL, -1 ) )
		{
			// szClass must be registered
			auto szName = pClass->GetClassName().c_str();
			luaL_error( pL, "PushToVM Class Not Registed:%s", szName );
			return;
		}

		lua_setmetatable( pL, -2 );

		// __cpp_object binds ObjectIndex
		lua_pushlightuserdata( pL, CScriptLua::ms_pCppObject );
		...
		void* pObject = *(void**)( pDataBuf );
		CScriptLua::RegisterObject( pL, pClass, pObject, nWeakTable );
		ConstructLua( pL );
	}
	...
	void CLuaValueObject::GetFromVM( UPakDataType Type, 
		lua_State* pL, char* pDataBuf, int32 nStkId, int32 nWeakTable )
	{
		CLuaObject::GetFromVM( Type, pL, pDataBuf, nStkId, nWeakTable );
	}

	void CLuaValueObject::PushToVM( UPakDataType Type, 
		lua_State* pL, char* pDataBuf, int32 nWeakTable )
	{
		auto pClass = Type.GetClass();
		if( !pClass->IsCopyConstructible() )
		{
			lua_pushnil( pL );
			return;
		}
		...
		CScriptLua::SRaiiLua Auto( pL );

		void* pNewObj = CScriptLua::NewLuaObj( pL, pClass );
		CScriptLua* pScriptLua = Auto.m_pScript;
		pClass->Construct( eBCI_CopyConstructor, pNewObj, (void** )&pDataBuf );

		// stack top = 2, object pointer is at top, table saving the object is below
		CScriptLua::RegisterObject( pL, pClass, pNewObj, nWeakTable );
		ConstructLua( pL );
	}

	//=====================================================================
	// Lua data type: C++ pointer type
	//=====================================================================
	class CLuaBuffer
	{
	public:
	...
	int32 CLuaBuffer::ReadBytes( lua_State* pL )
	{
		...
		{
			uint32 nReadCount = nArg >= 2 ? (uint32)GetNumFromLua( pL, 2 ) : UINT32_MAX;

			if( nReadCount == UINT32_MAX )
		...
		else
		{
			...
			uint32 nReadCount = nArg >= 4 ? (uint32)GetNumFromLua( pL, 4 ) : UINT32_MAX;
	...
	{
		uint32 nArg = lua_gettop( pL );
		uint32 nLen = (uint32)lua_rawlen( pL, 2 );
		const char* szString = lua_tostring( pL, 2 );
	...
	int32 CLuaBuffer::WriteBytes( lua_State* pL )
	{
		...
		if( lua_type( pL, 2 ) != LUA_TTABLE )
		{
			...
			uint32 nOffset = nArg >= 3 ? (uint32)GetNumFromLua( pL, 3 ) : 0;
			uint32 nWriteCount = nArg >= 4 ? (uint32)GetNumFromLua( pL, 4 ) : UINT32_MAX;

			if( nWriteCount == UINT32_MAX )
		...
		else
		{
			...
			uint32 nWriteCount = nArg >= 4 ? (uint32)(int64)GetNumFromLua( pL, 4 ) : UINT32_MAX;
	...
	int32 CLuaBuffer::Clear( lua_State* pL )
	{
		...
		if( nClearCount == UINT32_MAX || nArg < 2 )
	...
	void RegisterPointerClass( CScriptLua* pScript )
	{
		...
		lua_pushlightuserdata( pL, (void*)s_szLuaBufferClass );
		lua_pushvalue( pL, -2 );
	...
	bool PushPointerToLua( lua_State* pL, 
		void* pBuffer, int32 nWeakTable, bool bCreateStreamBuff )
	{
		if( pBuffer == nullptr )
		{
			lua_pushnil( pL );
		...
		if( type != LUA_TNIL )
		{
			if( type == LUA_TTABLE )
			{
				...
				{
					pInfo->nDataSize = (uint32)UINT32_MAX;
					pInfo->nCapacity = (uint32)UINT32_MAX;
					pInfo->nPosition = 0;
					return true;
				}
			}
		...
		lua_pushlightuserdata( pL, (void*)s_szLuaBufferClass );
		lua_rawget( pL, LUA_REGISTRYINDEX );
		if( lua_isnil( pL, -1 ) )// szClass must be registered
		{
