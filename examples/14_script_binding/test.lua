...
function CApplicationHandler:OnTestPureVirtual(
	e, v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12)
...
g_handler = CApplicationHandler();
g_App = ClassCast( CApplication, Base.CApp.GetInstance() );
Test( g_App == CApplication.GetInst(), "Test ClassCast" );

local address = SAddress(3, 5);
address.nIP = 123456789;
address.nPort = 1234;

local config = SApplicationConfig();
...
function StartApplication( name, id )
	...
	Base.CApp.GetInstance();

	local mapTest = g_App:TestTemplateMap( 987654, "TestMapValue" );
	--local Begin = mapTest:begin():Value();
	--Test( Begin.first == 987654 and Begin.second:c_str() == "TestMapValue", "Test Template iterator" );
	Test( config.Address.nIP == 123456789 and config.Address.nPort == 1234, "Test object value member" );
	Test( config.Address.port == 1234, "Test object value property" );
	...
	Test( g_App:TestCallPOD(1234, -123, -12345, -12345678, -1234567891011, -123456789, 123, 12345, 
		12345678, 1234567891011, 123456789, 1234567, 123456789101112, "abcdefg") == "OK",
		"Test return string" );
